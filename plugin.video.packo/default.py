...
ver_intro = True
base_server = 'http://89.163.212.78:8010/'
...
def abrir_cookie(url, New=False):
	...
	cj = cookielib.LWPCookieJar()
        br.set_cookiejar(cj)
	if not New:
	    cj.load(os.path.join(xbmc.translatePath("special://temp"),"addon_cookies_packo"), ignore_discard=False, ignore_expires=False)
	    ...
	    br.submit()
	    cj.save(os.path.join(xbmc.translatePath("special://temp"),"addon_cookies_packo"))
        br.open(url)
...
def Menu_Inicial_Filmes():
    addDir("Vod", "1", 201, "http://icons.iconarchive.com/icons/hadezign/hobbies/256/Movies-icon.png")
...
def Menu_Inicial_Series():
    ...
    addDir("Vod", "1", 601, "http://icons.iconarchive.com/icons/hadezign/hobbies/256/Movies-icon.png")
    xbmcplugin.setContent(int(sys.argv[1]), 'Movies')
    xbmc.executebuiltin("Container.SetViewMode(51)")
...
def addDir(name, url, mode, iconimage, total=0, pasta=True, plot='', fanart=''):
    ...
    contextMenuItems = []
    contextMenuItems.append(('Information', 'XBMC.Action(Info)'))
